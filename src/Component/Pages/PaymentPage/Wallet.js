...
export default function PaymentMethodPage() {
  ...
  useEffect(() => {
    ...
    const fetchWalletAndUser = async () => {
      try {
        const walletRes = await axios.get(`https://fliplyn-api.onrender.com/wallets/${userId}`);
        setWalletBalance(walletRes.data.balance_amount);

        const userRes = await axios.get(`https://fliplyn-api.onrender.com/user/${userId}`);
        setUserDetails({
    ...
    const fetchCartItems = async () => {
      try {
        const res = await axios.get(`https://fliplyn-api.onrender.com/cart/${userId}`, {
...
const handleConfirmPayment = async () => {
  ...
  try {
    const res = await axios.post('https://fliplyn-api.onrender.com/orders/place', requestBody);
    console.log('Order placed:', res.data);

    // ✅ Clear the cart after order success
    await axios.delete(`https://fliplyn-api.onrender.com/cart/clear/${userId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  return (
    <>
      ...
      <div className="payment-page-container">
        ...
        <p className="payment-subtitle">5 payment methods available</p>
        ...
        <div className="wallet-section">
          ...
          <p className="insufficient-text">
            You have insufficient balance. Please add funds to your wallet to complete the payment.
          </p>
      <button
